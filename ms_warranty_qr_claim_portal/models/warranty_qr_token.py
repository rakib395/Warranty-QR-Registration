...
class WarrantyQRToken(models.Model):
    ...
    purpose = fields.Selection([
        ...
        ('generic', 'Generic')
    ], string='Purpose', default='registration', tracking=True)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if serial_id:
                ...
                if reg:
                    vals['state'] = 'used'
                    vals['registration_id'] = reg.id

            product_id = vals.get('product_id')
            if product_id:
                product = self.env['product.product'].browse(product_id)
                if product and not product.product_tmpl_id.warranty_policy_id:
                    raise ValidationError(_(
                        "Error! Cannot generate QR Token. Product '%s' "
                        "does not have a Warranty Policy configured."
                    ) % product.name)
    ...
    def _check_product_warranty_policy(self):
        for rec in self:
            if rec.product_id:
                policy = rec.product_id.product_tmpl_id.warranty_policy_id
                if not policy:
                    raise ValidationError(_(
                        "Error! The product '%s' does not have a Warranty Policy configured. "
                        "Please set a warranty policy in the product template first."
                    ) % rec.product_id.name)
    ...
    def _compute_used_serial_ids(self):
        for record in self:
            if record.product_id:
                ...
                registered_serial_ids = self.env['ms.warranty.registration'].sudo().search(reg_domain).mapped('serial_no.id')

                claim_domain = [
                    ('replacement_product_id', '=', record.product_id.id),
                    ('replacement_serial_no', '!=', False),
                    ('state', 'not in', ['draft', 'rejected'])
                ]
                replaced_serial_ids = self.env['ms.warranty.claim'].sudo().search(claim_domain).mapped('replacement_serial_no.id')

                all_used_ids = list(set(token_used_serials + registered_serial_ids + replaced_serial_ids))
    ...
    def _onchange_product_id(self):
        ...
        used_token_ids = self.env['ms.warranty.qr.token'].sudo().search([
            ('product_id', '=', self.product_id.id),
            ('serial_no', '!=', False)
        ]).mapped('serial_no.id')

        used_reg_ids = self.env['ms.warranty.registration'].sudo().search([
            ('product_id', '=', self.product_id.id),
            ('serial_no', '!=', False),
            ('state', 'not in', ['rejected'])
        ]).mapped('serial_no.id')

        used_claim_ids = self.env['ms.warranty.claim'].sudo().search([
            ('replacement_product_id', '=', self.product_id.id),
            ('replacement_serial_no', '!=', False),
            ('state', 'not in', ['draft', 'rejected'])
        ]).mapped('replacement_serial_no.id')

        final_used_serial_ids = list(set(used_token_ids + used_reg_ids + used_claim_ids))

        final_serial_domain = [
            ('product_id', '=', self.product_id.id),
            ('id', 'not in', final_used_serial_ids)
        ]
