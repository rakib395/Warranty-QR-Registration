...
class WarrantyRegistration(models.Model):
    ...
    partner_id = fields.Many2one('res.partner', string='Customer Link', tracking=True) 
    ...
    def action_approve(self):
        ...
        for record in self:
            if record.serial_no:
                record.serial_no._compute_linked_registration()
                record.serial_no._compute_live_warranty_status()
    ...
    def action_reject(self):
        self.ensure_one()
        return {
            'name': _('Reject Warranty Request'),
            'type': 'ir.actions.act_window',
            'res_model': 'warranty.reject.reason.wizard',
            'view_mode': 'form',
            'target': 'new',
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            purchase_date_str = vals.get('purchase_date')
            
            if policy_id and purchase_date_str:
                policy = self.env['ms.warranty.policy'].browse(policy_id)
                purchase_date = fields.Date.from_string(purchase_date_str)
                
                if policy and purchase_date:
                    duration = policy.warranty_duration_value
                    unit = policy.warranty_duration_unit
                    expiry_date = False
                    
                    if unit == 'days':
                        expiry_date = purchase_date + timedelta(days=duration)
                    elif unit == 'months':
                        expiry_date = purchase_date + timedelta(days=duration * 30)
                    elif unit == 'years':
                        expiry_date = purchase_date + timedelta(days=duration * 365)
                        
                    if expiry_date and expiry_date < date.today():
                        vals['state'] = 'pending'

        records = super(WarrantyRegistration, self).create(vals_list)

        for reg in records:
            if reg.serial_no:
                reg.serial_no._compute_linked_registration()
                reg.serial_no._compute_live_warranty_status()
